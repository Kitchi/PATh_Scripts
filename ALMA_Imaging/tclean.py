...
if __name__ == '__main__':
    ...
    for nn, infile in enumerate(inpfiles):
        ...
        basefile = os.path.basename(infile.split('?')[0])
        if not os.path.exists(basefile):
            print(f"File {basefile} does not exist. Skipping.")
            continue

        spw = re.findall(r'spw\d+', basefile)
        if len(spw) > 0:
            spw = int(spw[0].strip('spw'))
        else:
            spw = -1

        chan = re.findall(r'chan\d+', basefile)
        if len(chan) > 0:
            chan = int(chan[0].strip('chan'))
        else:
            chan = -1
        ...
        input_MS = os.path.basename(basefile).strip('.tar.gz')
